    ...
    public void onTick(Minecraft mc) {
        ...
        if (setting("entity-push")) {
            for (Entity entity : mc.level.getEntities(mc.player, mc.player.getBoundingBox().inflate(0.3))) {
                ...
                if (!setting("apply-to") && !(entity instanceof Player)) {
                    continue;
                }
        ...
        if (setting("fishing-rod")) {
            for (net.minecraft.world.entity.projectile.FishingHook hook : mc.level.getEntitiesOfClass(
                    net.minecraft.world.entity.projectile.FishingHook.class,
                    mc.player.getBoundingBox().inflate(32))) {
                if (hook.getPlayerOwner() != mc.player && hook.getHookedIn() == mc.player) {
                    Vec3 v = mc.player.getDeltaMovement();
