...
| `src/client/java/fun/nursultan/client/mixin/` | Blink, NoServerRotation, NoEntityTrace, NoInteract, LockSlots, NoDelay, Fog, Removals overlays/Gui, AspectRatio, FreeLook, NoSlow, NoPush |
| `src/client/java/fun/nursultan/client/config/ConfigStore.java` | `zf` auto-save-preset (`nursultan-preset.json`) |
...
Оригинальный `Gs` рисует через MC RenderPipeline — без игры не встанет. Здесь меню, HUD (`td`) и все **109** UM-тиков завязаны на Mojang mappings 1.21.11. Пресет `zf` пишется в `nursultan-preset.json`. Mixin-слой: Blink, NoServerRotation, NoEntityTrace, NoInteract, NoFriendDamage, AntiBot, LockSlots, NoDelay, Fog, Removals (включая Gui scoreboard/vignette/effects), AspectRatio, FreeLook, NoSlow, NoPush.
